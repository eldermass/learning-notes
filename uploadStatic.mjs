...
async function main() {
  // 首先上传不带 hash 的文件
  for await (const entry of readdirp('./docs/.vuepress/dist', { type: 'files', directoryFilter: ['!assets'] })) {
    queue.add(() => uploadFile(entry.path))
    // uploadFile(entry.path)
  }
  // 上传携带 hash 的文件
  for await (const entry of readdirp('./docs/.vuepress/dist/assets', { type: 'files' })) {
